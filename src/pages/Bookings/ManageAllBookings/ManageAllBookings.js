...
const ManageAllBookings = () => {
    ...
    useEffect(() => {
        fetch('https://desolate-shelf-59541.herokuapp.com/.com/bookingRequests')
            .then(res => res.json())
            .then(data => setReqBookings(data));
    }, []);

    const handleUpdate = id => {
        fetch(`https://desolate-shelf-59541.herokuapp.com/.com/status/${id}`, {
            method: "PUT",
            headers: {
    ...
    const handleDelete = id => {
        console.log('id', id);
        const url = `https://desolate-shelf-59541.herokuapp.com/bookingRequests/${id}`;
        fetch(url, {
            method: 'DELETE'
        })
            .then(res => res.json())
